...
login.prototype.userInfo = function(params,callback)
{
  if (params.token)
  {
    /*
      ...
      WHERE (token = $1)
    */
    var queryVal = [{value: params.token}];
    var queryOpt =
    {
      queryName: "UserInfo",
    ...
    self.crud.select(queryOpt,queryVal,function(err,res)
    {
      ...
      if (res.result)
      {
        ...
        {
          var noUserErr = {
            message: "No user founded with session token "+params.token
          };
        ...
        {
          var moreErr = {
            message: "Founded more user with session token "+params.token
          };
